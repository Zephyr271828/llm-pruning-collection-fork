    ...
    if os.path.exists(targdir) and len(os.listdir(targdir)) > 0:
        oldest = min(
            [
                os.path.join(targdir, x)
                for x in os.listdir(targdir)
                if qualifier(os.path.join(targdir, x)) and
                os.path.isdir(os.path.join(targdir, x))
            ],
            key=key,
        )
        return oldest
...
class Checkpointer:
    ...
    def _write(self, state_dict, loader_state, process_group, save_name, rank):
        ...
        if loader_state is not None:
            torch.save(loader_state, os.path.join(save_name, f"dataloader_rank_{rank}.pth"))
        ...
        if load_path is None:
            ...
            if os.path.isfile(load_path):
                ...
                if dataloader is not None:
                    ...
                    dataloader_state = torch.load(os.path.join(load_path, f"dataloader_rank_{self.rank}.pth"), weights_only=True, map_location="cpu")
    ...
    def save_single_file(
        ...
        **kwargs,
        ...
        # Note: metadata kwargs cannot contain any of:
        # (step, model)
        single_files_dir = self.ckp_path.replace('checkpoints/', 'single_files/')
        os.makedirs(single_files_dir, exist_ok=True)
        save_name = os.path.join(single_files_dir, "step_" + str(step) + "_ckp.pth")
        save_time = time.time()
