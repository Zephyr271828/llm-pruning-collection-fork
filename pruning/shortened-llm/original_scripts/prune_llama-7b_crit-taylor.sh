...
# export BASE_MODEL=baffo32/decapoda-research-llama-7B-hf
export BASE_MODEL='/n/fs/vision-mix/yx1168/model_ckpts/Llama-2-7b-hf'
export MODEL_NAME=llama-1-7b
...
python src/anal_block_sensitivity_taylor.py \
    ...
    --output_dir $OUTPUT_SENSITIVITY --fix_decapoda_config
...
python src/block_prune.py \
    ...
    --output_dir $OUTPUT_PRUNE --fix_decapoda_config

# if [ "$BASE_MODEL" = "baffo32/decapoda-research-llama-7B-hf" ]; then
#     python src/copy_tokenizer_decapoda-research-llama-7B-hf.py \
...
#     --save_lora_merge --fix_decapoda_config

# if [ "$BASE_MODEL" = "baffo32/decapoda-research-llama-7B-hf" ]; then
#     python src/copy_tokenizer_decapoda-research-llama-7B-hf.py \
#         --output_dir ${OUTPUT_TUNE}_lora_merge_fp16
# fi    

# # Compute Zero-shot PPL on WikiText2 and PTB 
# python src/eval_ppl.py \
#     --base_model ${OUTPUT_TUNE}_lora_merge_fp16 \
#     --output_dir ${OUTPUT_TUNE}_score --fix_decapoda_config
