...
def load_and_replace_weights(
    ...
    prune_config_path = f"{pruned_weights_path}/prune_config.json"
    if os.path.exists(prune_config_path):
...
if __name__ == "__main__":
    base_model_path = "/scratch/yx3038/cache/Llama-3.1-8B"
    pruned_weights_paths = (
        "/scratch/yx3038/pruning/llm-pruning-collection-fork-flap/pruning/FLAP/llm_weights/flap_p0.2_WIFV_AL-AM_llama_7b",
        "/scratch/yx3038/pruning/llm-pruning-collection-fork-flap/pruning/FLAP/llm_weights/flap_p0.2_WIFV_AL-AM_llama3.1_8b",
        "/scratch/yx3038/pruning/llm-pruning-collection-fork-flap/pruning/FLAP/llm_weights/flap_p0.3_WIFV_AL-AM_llama3.1_8b",
        "/scratch/yx3038/pruning/llm-pruning-collection-fork-flap/pruning/FLAP/llm_weights/flap_p0.5_WIFV_AL-AM_llama3.1_8b",
    )
    
    for pruned_weights_path in pruned_weights_paths:
        out_path = f"{pruned_weights_path}/huggingface"
        if os.path.exists(out_path):
            print(f"Output path {out_path} already exists, skipping...")
            continue
        model = load_and_replace_weights(base_model_path, pruned_weights_path, overwrite_config=True)
