...
def _get_head_dim(layer):
    if hasattr(layer.self_attn, "head_dim"):
        return int(layer.self_attn.head_dim)
    return int(layer.self_attn.hidden_size // layer.self_attn.num_heads)


def _get_gqa_groups(layer):
    if hasattr(layer.self_attn, "num_key_value_groups"):
        return int(layer.self_attn.num_key_value_groups)
    if hasattr(layer.self_attn, "num_key_value_heads") and layer.self_attn.num_key_value_heads:
        return int(layer.self_attn.num_heads // layer.self_attn.num_key_value_heads)
    return 1
...
def compress(layer, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device, bias=True, unstr=False):
    ...
    if unstr:  # Only mask, do not really prune
        ...
        if attn_mask is not None:
            ...
            retain_heads_qo = retain_heads_kv * int(gqa_groups)
            attn_mask_kv = attn_mask.repeat_interleave(head_dim)
            attn_mask_qo = attn_mask.repeat_interleave(gqa_groups).repeat_interleave(head_dim)
            # Apply the mask to the query, key and value projection weights
            layer.self_attn.q_proj.weight.data *= attn_mask_qo.unsqueeze(-1).to(device)
            layer.self_attn.k_proj.weight.data *= attn_mask_kv.unsqueeze(-1).to(device)
            layer.self_attn.v_proj.weight.data *= attn_mask_kv.unsqueeze(-1).to(device)
        ...
        if attn_mask is not None:
            ...
            layer.self_attn.num_heads = int(retain_heads_qo)
            layer.self_attn.hidden_size = int(retain_heads_qo) * int(head_dim)
            # Update num_key_value_heads for both GQA and MHA cases
            if hasattr(layer.self_attn, "num_key_value_heads"):
                layer.self_attn.num_key_value_heads = int(retain_heads_kv)
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if name == 'self_attn.o_proj':
                head_dim = _get_head_dim(layer)
                gqa_groups = _get_gqa_groups(layer)
                W_metric = W_metric.reshape(-1, head_dim * gqa_groups).sum(dim=1) # importance score of each head
