...
class L0Module(nn.Module):
    def __init__(self, cfg, device):
        ...
        self.pruning_modules = [
            "qo_head_dim" if m == "qk_head_dim" else "kv_head_dim" if m == "vo_head_dim" else m
            for m in l0_module_cfg.pruning_modules
        ]
    ...
    def set_model_info(self, cfg, n_matrix_mlp):
        ...
        ns.num_key_value_heads = cfg.get('n_kv_heads', cfg.get('num_key_value_heads', cfg.n_heads))
        ns.mlp_num_per_layer = 1
        ns.dim_per_head = cfg.get('head_dim', ns.hidden_size // ns.num_attention_heads)
        ns.num_layers = cfg.n_layers
        ns.vocab_size = cfg.vocab_size
        ns.q_hidden_size = ns.num_attention_heads * ns.dim_per_head
        ns.kv_hidden_size = ns.num_key_value_heads * ns.dim_per_head

        ns.params_per_head_layer = (
            ns.hidden_size * ns.q_hidden_size
            + ns.hidden_size * ns.kv_hidden_size
            + ns.hidden_size * ns.kv_hidden_size
            + ns.q_hidden_size * ns.hidden_size
        )
        ns.params_per_head =  ns.params_per_head_layer // ns.num_key_value_heads
    ...
    def initialize_head(self):
        mask_shape = [self.base_model_info.num_layers, self.base_model_info.num_key_value_heads]
        num_params_per_mask = self.base_model_info.params_per_head
        mask_output_shape = [self.base_model_info.num_layers, 1, self.base_model_info.num_key_value_heads, 1] 
        ...
        if self.target_model_info is not None:
            ...
            target_mask_size = self.target_model_info.num_key_value_heads
    ...
    def initialize_qo_head_dim(self): # only compatible when target model info is available
        mask_shape = [self.base_model_info.num_layers, self.base_model_info.num_key_value_heads, self.base_model_info.dim_per_head]
        num_params_per_mask = 2 * self.base_model_info.hidden_size
        mask_output_shape = [self.base_model_info.num_layers, self.base_model_info.kv_hidden_size] 
        
        target_qo_head_dim_sparsity = None; pd = {}; target_mask_size = None
        if self.target_model_info is not None:
            target_qo_head_dim_sparsity = 1 - self.target_model_info.kv_hidden_size / self.base_model_info.kv_hidden_size
            target_mask_size = self.target_model_info.kv_hidden_size
            pd = {"lambda_1_qo_head_dim": torch.nn.Parameter(torch.tensor(0.0, device=self.device)),
                  "lambda_2_qo_head_dim": torch.nn.Parameter(torch.tensor(0.0, device=self.device))}
            self.lambdas.update(pd)
        qo_head_dim = Mask(name="qo_head_dim",
                         mask_shape=mask_shape,
                         num_params_per_mask=num_params_per_mask,
                         mask_output_shape=mask_output_shape,
                         target_sparsity=target_qo_head_dim_sparsity,
                         target_mask_size=target_mask_size,
                         device=self.device,
                         eval_target_model=self.eval_target_model)
        self.masks["qo_head_dim"] = qo_head_dim 
          
          
    def initialize_kv_head_dim(self): # only compatible when target model info is available
        mask_shape = [self.base_model_info.num_layers, self.base_model_info.num_key_value_heads, self.base_model_info.dim_per_head]
        num_params_per_mask = 2 * self.base_model_info.hidden_size
        mask_output_shape = [self.base_model_info.num_layers, self.base_model_info.kv_hidden_size] 
        
        target_kv_head_dim_sparsity = None; pd = {}; target_mask_size = None
        if self.target_model_info is not None:
            target_kv_head_dim_sparsity = 1 - self.target_model_info.kv_hidden_size / self.base_model_info.kv_hidden_size
            target_mask_size = self.target_model_info.kv_hidden_size
            pd = {"lambda_1_kv_head_dim": torch.nn.Parameter(torch.tensor(0.0, device=self.device)),
                  "lambda_2_kv_head_dim": torch.nn.Parameter(torch.tensor(0.0, device=self.device))}
            self.lambdas.update(pd)
        kv_head_dim = Mask(name="kv_head_dim",
                         mask_shape=mask_shape,
                         num_params_per_mask=num_params_per_mask,
                         mask_output_shape=mask_output_shape,
                         target_sparsity=target_kv_head_dim_sparsity,
                         target_mask_size=target_mask_size,
                         device=self.device,
                         eval_target_model=self.eval_target_model)
        self.masks["kv_head_dim"] = kv_head_dim 
        
    def initialize_head_layer(self):
        mask_shape = [self.base_model_info.num_layers]
        num_params_per_mask=self.base_model_info.params_per_head *  self.base_model_info.num_key_value_heads
    ...
    def initialize_layer(self):
        mask_shape = [self.base_model_info.num_layers]
        num_params_per_mask=self.base_model_info.params_per_head * self.base_model_info.num_key_value_heads + self.base_model_info.params_per_mlp_layer
        mask_output_shape = [self.base_model_info.num_layers] 
    ...
    def get_expected_num_params(self, expected_scores: dict): #! calculate the current parsity
        ...
        qo_score = None
        if "qo_head_dim" in expected_scores:
            qo_head_dim_score = expected_scores["qo_head_dim"]
            kv_head_dim_score = expected_scores["kv_head_dim"]
            qo_head_dim_score = qo_head_dim_score.view(qo_head_dim_score.shape[0], -1)
            kv_head_dim_score = kv_head_dim_score.view(kv_head_dim_score.shape[0], -1)
            head_score = torch.repeat_interleave(head_score, self.base_model_info.dim_per_head, dim=1) # 12 * 768

            qo_score = head_score * qo_head_dim_score
            kv_score = head_score * kv_head_dim_score
        ...
        if "hidden" in expected_scores:
            ...
            if qo_score is None:
                ...
                num_parameters += torch.sum(torch.matmul(hidden_score.reshape(1, -1, 1), qo_score.unsqueeze(1))) * 2
                num_parameters += torch.sum(torch.matmul(hidden_score.reshape(1, -1, 1), kv_score.unsqueeze(1))) * 2
